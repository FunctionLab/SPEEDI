...
#' @return TRUE
#' @export
preliminary_check_for_SPEEDI_errors <- function(reference_tissue, data_type = "RNA", species = "human", input_dir = getwd(), reference_dir = getwd(), output_dir = getwd(), metadata_df = NULL, reference_file_name = NULL, reference_cell_type_attribute = "celltype.l2", analysis_name = NULL, sample_id_list = NULL, sample_file_paths = NULL, record_doublets = FALSE, exit_with_code = FALSE, log_flag = FALSE) {
  exit_code <- -1
  ...
  if(is.null(input_dir) & is.null(sample_file_paths)) {
    print_SPEEDI("Error: You must provide a value for \"input_dir\" if you do not provide a value for \"sample_file_paths\".", log_flag = log_flag)
    exit_code <- 7
  }
