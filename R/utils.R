...
#' @param verbose whether to give detailed feedback
#' @return stuff
#' @importFrom Seurat %||%
PrepDR <- function(object, features = NULL, slot = 'scale.data', verbose = TRUE) {
  if (length(x = Seurat::VariableFeatures(object = object)) == 0 && is.null(x = features)) {
...
PCA <- function(sc_obj) {
  ...
  system.time(pca_results <- irlba::prcomp_irlba(x = t(data.use), n = 30, retx = TRUE, center = TRUE, scale. = FALSE))
  ...
  reduction.data <- Seurat::CreateDimReducObject(
    ...
    assay = Seurat::DefaultAssay(object = sc_obj),
    stdev = sdev,
    key = "PC_",
